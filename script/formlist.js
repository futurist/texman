  ...
  constructor () {
    ...
    self.controller = function (args) {
      this.updateStyle = function () {
        self.style = mj2c.sheet(self.css)
      }
    ...
    self.view = function (ctrl) {
      ...
      var dom = mc( '.!list',
                    ...
                    [ mc.style(self.style),
                      mc('.!operate', mc('a.add[href="cane.html"][target=_blank]', '添加')),
                      mc( 'ul',
                          data.map((v, i) => {
                            return mc( 'li',
                                       [ mc( '.item',
                                             [ mc( '.name',
                                                   { onclick: function () { showForm(v) } },
                                                   v.attributes.name
                                                 ),
                                               mc('.title', {onclick: function () {
                                                 showDataList(v)
                                               }}, v.attributes.title),
                                               mc('.createAt', v.attributes.createAt),
                                             ]
                                           ),
                                         mc('a.action[href="cane.html#id=' + v.id + '&ret=' + window.location.href + '"][target=_blank]', '编辑'),
                                         mc(`a.action[href="#${v.id}"]`, { onclick: function () {
...
class DataListView {
  constructor (formType, options = {}) {
    ...
    this.data_table_multi_view = mj2c.sheet({
    ...
    this.controller = function () {
      ...
      ctrl.saveList = function () {
        Object.keys(changedData).forEach(rowID => {
          ...
          var data = {}
          list.forEach(v => {
            data[v] = changedData[rowID][v].newVal})
          let apiData = {
            'data': {
              'type': name,
              'id': rowID,
      ...
      ctrl.buildTableRows = function (typeInfo, data) {
        var renderCell = function (row, key) {
          ...
          case 'edit':
            if (isTextArea) {
              return mc('textarea', Global._extend(Global.clone(typeInfo[key].attrs), { name: row.id + '_' + key, oninput: function () {
            ...
            return mc('input', Global._extend(Global.clone(typeInfo[key].attrs) , { name: row.id + '_' + key, value: row.attributes[key] || '', oninput: function () {
        ...
        var renderAction = function (row) {
          ...
          return mc('td.cell.action.first',
                   [
                     mc('a.action.icon[href="javascript:;"]', {onclick: function () { showForm(formType, {row: row}) }}, mc('i.iconfont.edit')),
                     mc('a.action.icon[href="javascript:;"]', {className: '', onclick: function () {
                       if ($(this).hasClass('deleting'))return
                       ctrl.deleteRow(row)
                       $(this).addClass('deleting'); }}, mc('i.iconfont.delete')),
                   ]
                  )
        }

        return data.data.map(function (v) {
          return mc('tr.data.row',
                   {
                   ...
                   [
                     ...
                     Object.keys(typeInfo).map(key => {
                       var isTextArea = typeInfo[key].tag == 'textarea'
                       var val = isTextArea
                         ? mc('textarea', typeInfo[key].attrs.html ? m.trust(v.attributes[key] || '') : (v.attributes[key] || ''))
                       : v.attributes[key]
                       return mc('td.cell.' + key, {
                         ...
                         config: function (el, old, context) {
                           if (old) return
                           setTimeout(function () {
                             $(el).addClass(self['data_table'].lastEditRowNormal)
                           }, 2000)
                           $(el).on('click', function (e) {
      ...
      ctrl.buildTableHeader = function (typeInfo) {
        return mc('th.head.row', [
          options.hideAction || options.readOnly ? [] : mc('td.cell.first', {style: {width: '76px'} }, mc('.actionHeader', 'action')),
          Object.keys(typeInfo).map((v, idx) => {
            ...
            return mc('td.cell.' + v, { title: v + '\n' + description},
                     [
                       title || v,
                       mc('i', {className: '!iconfont ' + (options.sort[v] > 0 ? 'down' : 'up') + 'arrow ' + (~options.sortField.indexOf(v) ? '' : ' grey'), onclick: function () {
                       ...
                       mc('.searchBox.!ui.!input', {className: $('[name=filter_' + v + ']').val() ? 'hasText' : '' },
                         [
                           mc('i', {class: 'iconfont clear', onclick: function () {
                             $('[name=filter_' + v + ']').val('')
                             delete options.filter[v]
                             $(this).parent().removeClass('hasSearched')
                             ctrl.updateListView()
                           }}),
                           mc('input[type=text][name=filter_' + v + ']', {
                             oninput: function () {
                               $(this).parent().removeClass('hasSearched')
                             },
                           ...
                           mc('i', {class: 'iconfont circular search link icon', onclick: function () {
                             ctrl.filterHistory(v, options.filter[v])
    ...
    this.view = function (ctrl) {
      return mc('.listCon',
        [
          mc.style(self.data_table),
          mc('.listAction',
            [
              listMode == 'edit'
                ? [
                  mc('input[type=button][value=' + '列表模式' + ']', {onclick: function () { listMode = 'text'; ctrl.updateListView() }}),
                  mc('input[type=button][value=' + '保存编辑' + ']', {onclick: function () { ctrl.saveList() }}),
                ]
              : (options.readOnly ? [] : mc('input[type=button][value=' + '编辑模式' + ']', {onclick: function () { listMode = 'edit'; ctrl.updateListView() }}))
            ]
           ),
          mc('table.table', {
            className: '!ui !table',
